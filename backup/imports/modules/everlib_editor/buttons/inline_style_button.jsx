...
class InlineStyleButton extends React.Component {
  ...
  isInlineStyleActive = (inlineStyle) =>
    this.context.editorState.getCurrentInlineStyle().has(inlineStyle) ? 'active' : '';

  renderLi = () => {
    return (
      <li
        className={`toolbar-btn ${this.isInlineStyleActive(this.props.inlineStyle)}`}
        onMouseDown={(event) => this.toggleInlineStyle(event, this.props.inlineStyle)} >
